...
export const reqCommonMethodNoLoad=(data,url)=>fetch(url,data,"POST")//无加载条通用请求
export const reqCommonMethod=(data,load,url)=>fetch(url,data,"POST",load)//通用请求方式
export const getAuditStatisticsYm=(data)=>fetch("km-ecs/w/audit/statisticsaudit",data,"POST",true);//获取YM审核统计
