...
<template>
  <div id="merchantSearch">
    ...
    <!--修改商户信息-->
    <ModifyInfo v-if="off.modifyInfo" :merchantInfo="ajaxData.details" v-on:details="details" ref="ModifyInfoCase">
    </ModifyInfo>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      init: function () {
        ...
        type == 'pending' ? vm.off.type = 1 : type == 'processing' ? vm.off.type = 2 : type == 'finish' ? vm.off.type =
          3 : void(0);
        ...
        requestGetMerchantList(searchData, function () {
          ...
          .then((data) => {
            if (data.code == 200) {
              ...
              vm.form.content == 1 ? vm.searchResultList.list = data.data.list : vm.searchResultList2.list = data.data
                .list;
              vm.form.content == 1 ? vm.searchResultList.maxpage = Math.ceil(parseInt(data.data.total) / 10) : vm.searchResultList2
                .maxpage = Math.ceil(parseInt(data.data.total) / 10);
              vm.form.content == 1 ? vm.searchResultList.pageNum = page || 1 : vm.searchResultList2.pageNum = page ||
                1;
              vm.form.content == 1 ? vm.searchResultList.callback = function (v) {
