...
<div style="width100%;height:100%;padding:10px;">
	<section class="g-audit-menu" v-show="!off.isGet">
		<div class="g-audit-menu-c" v-show="off.auditType==1">
			<div class="m-menu-div">
				<a href="javascript:;" @click="getList(1)" class="u-bg navBg10"><div>激活商户</div><p><span>点击审核</span></p></a>
			...
			<div class="m-menu-div">
				<a href="javascript:;" @click="getList(2)" class="u-bg navBg13"><div>商户实名补录</div><p><span>点击审核</span></p></a>
				<div class="navInfoDiv"><p>待审核<span>{{auditCount.fillMerchantRealTime}}</span></p><p>已分配<span>{{auditCount.fillMerchantRealTime_}}</span></p></div>
			</div>
		</div>
		<div class="g-audit-menu-c" v-show="off.auditType==2">
			<div class="m-menu-div">
				<a href="javascript:;" @click="getList(1)" class="u-bg navBg10"><div>激活商户</div><p><span>点击审核</span></p></a>
	...
	<div id="auditList" v-show="off.isGet">
	  <section class="g-list-box" v-if="auditData">
	  	  <header class="g-lis-head">
	  	  	...
	  	  	<div class="m-time-down" v-if="off.time">审核计时：<span class="f-c-red">{{off.time}}</span></div>
	  	  	<div class="m-foot-btn">
				<a class="f-btn f-btn-success" @click="agree">同意</a>
				<a class="f-btn f-btn-danger" @click="refuse">拒绝</a>
		  	</div>
	  	  </header>
	  	  <div class="g-box" v-show="off.type == 1">
				...
				<tbody>
					<tr>
						...
						<td class="m-box-img m-meida-640up">
							<ImgZoom :imgData="imgData" v-if="imgData.length"></ImgZoom>
						</td>
					</tr>
		  ...
		  <FillMerchant v-show="off.type == 2" :oldInfo="oldInfo" :newInfo="newInfo" :type="1"/>
	  </section>
...
<script>
import api from './api/registMerchant';
...
export default{
	...
	methods:{
		...
		agree:function(){//审核同意
			var vm=this,
				orderId = vm.off.type == 1 ? vm.auditData.orderId : vm.auditData.newInfo.orderId;
            ...
            api.actionAudit({
				...
				"gztResult":vm.auditData.gztResult || ''
			}, false, vm.off.type)
            .then((data)=>{
               layer.open({
					content:data.msg,
		...
		refuse:function(obj){//审核拒绝
			var vm=this,str='',
				orderId = vm.off.type == 1 ? vm.auditData.orderId : vm.auditData.newInfo.orderId,
				popIndex,ww=window.innerWidth,wwSet;
		...
		getAuditList:function(){//获取订单
			...
			api.getAuditList({"auditType":vm.off.auditType},function(){
				vm.off.isLoad=false;
			},vm.off.type)
			.then((data)=>{
				...
				if(vm.list[0].peirod)vm.timeDown(parseInt(vm.list[0].peirod));
				vm.off.isLoad=false;
			})
			.catch(error=>errorDeal(error)); 
		...
		dealAuditList:function(){//处理分配的订单
			...
			if(len&&(vm.off.auditIndex+1)<=len){
				...
				if(vm.off.type == 1){//激活商户
					...
					vm.oldInfo = vm.auditData.oldInfo;
				}
