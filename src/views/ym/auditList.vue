...
<template>
  <div id="auditList">
    ...
    >
      <header class="g-lis-head">
        ...
        <div
          class="m-time-down"
          style="display:none"
        >审核计时：<span class="f-c-red">{{off.time}}</span></div>
        <div
          class="m-foot-btn"
          :class="{scroll:off.isScroll}"
        >
          <a
      ...
      <div class="g-box">
        <table
          class="g-list-table"
          :class="{transfer:$route.params.type==3||$route.params.type==5}"
        >
          <tbody v-if="$route.params.type!=3&&$route.params.type!=5">
            <tr>
              <td>
                <table class="g-inner-table">
                  <tbody>
                    ...
                    <tr>
                      <td>生成时间：</td>
                      <td>{{getDateTime(auditData.create_time)[6]}}</td>
                    </tr>
                    <tr>
                      <td>用户姓名：</td>
                    ...
                    <tr>
                      <td>电话号码：</td>
                      <td>{{ auditData.phoneNumber }}</td>
                    </tr>
                    <tr>
                      <td>号码等级：</td>
                      <td>{{ typeCheck(4,auditData.phoneLevel) }}</td>
                    </tr>
                    <tr>
                      <td>{{ typeCheck(auditData.papersType)[2] }}：</td>
                      <td>{{ auditData.papersCode }}</td>
                    </tr>
                    <tr>
                      <td>证件类型：</td>
                      <td>{{ typeCheck(auditData.papersType)[0] }}</td>
                    </tr>
                    <tr>
                      <td>{{ typeCheck(auditData.papersType)[1] }}：</td>
                    ...
                    <tr>
                      <td>号码归属部门：</td>
                      <td>{{ auditData.phone_depName }}</td>
                    </tr>
                    <tr>
                      <td>UCCID：</td>
                      <td>{{ auditData.cardNumber }}</td>
                    </tr>
                    <tr>
                      <td>Mac地址：</td>
                    ...
                    <tr>
                      <td>识别模式：</td>
                      <td>{{ auditData.openMode }}</td>
                    </tr>
                    <tr>
                      <td>操作人：</td>
                      <td>{{ auditData.operatorName }}</td>
                    </tr>
                    <tr>
                      <td>操作人工号：</td>
                      <td>{{ auditData.operator }}</td>
                    </tr>
                    <tr>
                      <td>操作人部门：</td>
                      <td>{{ auditData.depName }}</td>
                    </tr>
                    <tr>
                      <td>备注：</td>
                      <td>{{ auditData.comment }}</td>
                    </tr>
                  </tbody>
                </table>
              </td>
        ...
        <div
          ...
          v-if="$route.params.type==3||$route.params.type==5"
        >
          <div class="m-zoomContent zoom-c">
            <div class="m-img-c">
              <div
    ...
    <div
      ...
      v-if="!auditData"
    ><span
        class="u-icon-refresh"
        :class="{rotate360:off.isLoad}"
      ></span><a @click="getAuditList">{{off.isLoad?'loading':'点击刷新'}}</a></div>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      refuseArr: [
        "没有用户签名或签名与用户姓名不符",
        "部分或全部照片中存在模糊、分辨率太小无法辨认/严重变形/反光严重/拍摄角度不规范",
        "部分/全部照片中存在非远特SIM卡号码现象",
        "部分/全部照片中的SIM卡号码与填报号码不符",
        "无证件与SIM卡合影(包括身份证正反面及现场手持照)",
        "背景非通讯营业网点、非通讯销售商业场所或非售卡现场拍摄照片(包括证件及手持照)",
        "手持证卡合影不规范,非免冠正脸照、人脸被遮挡等导致无法清晰辨别是否为本人",
        "同一证件、姿态手持合影照,与不同号码交叉反复提交",
        "同一证件为不同号码当日交叉提交达三次以上",
        "非有效期证件或申卡人未满16周岁",
        "填报信息与证照中信息不符、缺失内容或乱码(包括但不限于证件号码,姓名,地址等信息)",
        "发证机关与证件行政归属地不符",
        "部分或全部照片内容为疑似翻拍或合成",
        "非本人现场手持证卡照（严重违规）",
        "证件与手持合影照中证件不同（严重违规）",
        "已被拒绝并明知信息有问题仍然多次重复恶意上传（严重违规）",
        "部分或全部照片内容被明确为翻拍或合成、造假（严重违规）"
      ]
    };
  },
  created: function() {
    ...
    userInfo.isadmin && userInfo.isadmin.indexOf("4") > -1
      ? (this.off.isBugginPower = true)
      : (this.off.isBugginPower = false);
    this.getAuditList();
    this.dealAuditList();
  },
  mounted: function() {
  ...
  methods: {
    ...
    refuse: function(obj) {
      ...
      for (var i = 0; i < vm.refuseArr.length; i++) {
        ...
        str +=
          '<div class="checkbox-list"><div class="m-form-checkbox"><label><span class="checkbox"><input value="' +
          vm.refuseArr[i] +
          '" type="checkbox" name="refuse_model"><span></span></span><span class="text">' +
          vm.refuseArr[i] +
          b +
          "</span></label></div></div>";
      }
      ...
      popIndex = layer.open({
        content:
          '<div class="f-scroll-lt lay-div-refuse f-tal" id="refuseList">' +
          str +
          '<div class="checkbox-list"><input maxlength="30" type="text" id="reason" placeholder="请输入补充内容"></div></div>',
        btn: ["确定", "取消"],
        type: 1,
        style: wwSet,
        title: [
      ...
      if (len && vm.off.auditIndex + 1 <= len) {
        ...
        if (vm.$route.params.type == 3 || vm.$route.params.type == 5) {
          if(vm.$route.params.type == 3){//过户
            ...
            vm.imgData[6] = { src: vm.auditData.headImage || noImg, name: "新机主头像照片" };
            // vm.imgData[8] = { src: vm.auditData.signImageOld || noImg, name: "原机主手签名" }; 
            vm.imgData[7] = { src: vm.auditData.signImage || noImg, name: "新机主手签名" };
          }
          ...
          vm.imgData = [
            ...
            { src: vm.auditData.papersImage || noImg, name: "正面" },
            { src: vm.auditData.backImage || noImg, name: "反面" },
            { src: vm.auditData.headImage || noImg, name: "头像" },
            { src: vm.auditData.signImage || noImg, name: "手签名" }
          ];
    ...
    timeDown: function(time) {
      ...
      var vm = this,
        ...
        timer = setInterval(function() {
          time--;
          time == 0
            ? ((vm.off.time = "00:00"), clearInterval(timer), vm.getAuditList())
