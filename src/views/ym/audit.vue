	...
	<section class="g-audit-menu">
		<div v-if="off.source==0" class="g-audit-menu-c">
			<div class="m-menu-div">
				...
				<div class="navInfoDiv"><p>待审核<span>{{count.data.readyCard}}</span></p><p>已分配<span>{{count.data.readyCard_1}}</span></p></div>
			</div>
			<div class="m-menu-div">
				<router-link :to="{name:'card',params:{type:2}}" class="u-bg navBg3"><div>白卡审核</div><p><span>点击审核</span></p></router-link>
				<div class="navInfoDiv"><p>待审核<span>{{count.data.whiteCard}}</span></p><p>已分配<span>{{count.data.whiteCard_1}}</span></p></div>
			</div>
			<div class="m-menu-div">
				<router-link :to="{name:'card',params:{type:3}}" class="u-bg navBg5"><div>过户办理</div><p><span>点击审核</span></p></router-link>
				<div class="navInfoDiv"><p>待审核<span>{{count.data.newChangeCard}}</span></p><p>已分配<span>{{count.data.newChangeCard_1}}</span></p></div>
			</div>
			<div class="m-menu-div">
				<router-link :to="{name:'card',params:{type:4}}" class="u-bg navBg4"><div>实名补录</div><p><span>点击审核</span></p></router-link>
				<div class="navInfoDiv"><p>待审核<span>{{count.data.realNameCollection}}</span></p><p>已分配<span>{{count.data.realNameCollection_1}}</span></p></div>
			</div>
			<div class="m-menu-div">
				<router-link :to="{name:'card',params:{type:5}}" class="u-bg navBg6"><div>实名登记</div><p><span>点击审核</span></p></router-link>
				<div class="navInfoDiv"><p>待审核<span>{{count.data.fill}}</span></p><p>已分配<span>{{count.data.fill_1}}</span></p></div>
			</div>
		</div>
