...
export default{
	...
	methods:{
		...
		getDetails(isBtn){//获取商户/工号基本信息
			...
			type==1 ? (json={dealerId:vm.form.context},url='w/merchant/getInfo') : (json={phone:vm.form.context},url='w/user/getInfo');
		...
		getList(page){//获取第三方支付订单列表/获取用户列表
			...
			type==1 ? (json={dealerId:vm.form.context},url='w/audit/getUsersDetail') : (json={phone:vm.form.context,pageNum:page||1,pageSize:10,type:vm.form.paySource},url='w/user/paymengList');
