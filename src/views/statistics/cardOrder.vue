...
export default{
	...
	methods:{
		...
		orderListDown:function(){
			...
			vm.tabType==2 ? url='w/audit/orderListDown' : url='w/audit/getFreezonOrder'
		...
		dealerListDown:function(){
			...
			vm.AJAX('w/audit/dealerListDown',json,function(data){
				var frame=document.getElementById('downloadFrame');
				frame.setAttribute('src',data.data.url);
			})
		},
		accountList:function(){//账户对账明细
			...
			vm.AJAX('w/audit/accountList',json,function(data){
				var frame=document.getElementById('downloadFrame');
				frame.setAttribute('src',data.data.url);
			});
		},
